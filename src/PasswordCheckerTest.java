...
public class PasswordCheckerTest {
	...
	public void setUp() throws Exception {
		...
		*/
		String[] f = {"5709CC", "Th1s", "Work55", "Asitshould", "proj3ctby", "Yourstruly"};
		STUDENTpasswords = new ArrayList<String>();
		STUDENTpasswords.addAll(Arrays.asList(f));
	}
	...
	{
		try{
			PasswordCheckerUtility.isValidPassword("frank4908");
	...
	public void testIsValidPasswordSuccessfulSTUDENT()
	{
		try{
			PasswordCheckerUtility.isValidPassword("Frank4908");
			PasswordCheckerUtility.isValidPassword("This5hould");
			PasswordCheckerUtility.isValidPassword("Workwe11");
	...
	{
		ArrayList<String> STUDENTresults;
		STUDENTresults = PasswordCheckerUtility.validPasswords(STUDENTpasswords);
		Scanner scan = new Scanner(STUDENTresults.get(0));
		assertEquals(scan.next(), "5709CC");
		String line = scan.nextLine().toLowerCase();
		assertTrue(line.contains("lowercase"));
		
		scan = new Scanner(STUDENTresults.get(1));
		assertEquals(scan.next(), "Th1s");
		line = scan.nextLine().toLowerCase();
		assertTrue(line.contains("characters"));
		
		scan = new Scanner(STUDENTresults.get(2));
		assertEquals(scan.next(), "Asitshould");
		line = scan.nextLine().toLowerCase();
		assertTrue(line.contains("digit"));
		
		scan = new Scanner(STUDENTresults.get(3));
		assertEquals(scan.next(), "proj3ctby");
		line = scan.nextLine().toLowerCase();
		assertTrue(line.contains("uppercase"));
		
		scan = new Scanner(STUDENTresults.get(4));
		assertEquals(scan.next(), "Yourstruly");
		line = scan.nextLine().toLowerCase();
		assertTrue(line.contains("digit"));
